...
class G1Env(HumanoidEnv):
    def __init__(
        ...
        **kwargs,
        ...
        self.with_hands = bool(config.get("with_hands", False))
        if config.get("ENV_TYPE") == "real" and self.with_hands:
            raise RuntimeError(
                "real G1 Decoupled WBC requires with_hands=False; "
                "Dex3 sender/calibration must not be constructed"
            )
        ...
        if self.with_hands:
            self._hands = Hands()
            self._hands.left = G1ThreeFingerHand(
                is_left=True,
                env_type=config["ENV_TYPE"],
            )
            self._hands.right = G1ThreeFingerHand(
                is_left=False,
                env_type=config["ENV_TYPE"],
            )
    ...
    def arm_body_writes(self, *, takeover_confirmed: bool) -> None:
        self.body().body_command_sender.arm_writes(takeover_confirmed=takeover_confirmed)

    def disarm_body_writes(self) -> None:
        self.body().body_command_sender.disarm_writes()
