...
class BodyCommandSender:
    def __init__(self, config: Dict):
        self.config = config
        if self.config.get("ENV_TYPE") == "real":
            raise RuntimeError(
                "BodyCommandSender is disabled on real hardware; "
                "UnitreeLowCmdWriter in real_safe.lowcmd_guard is the only permitted rt/lowcmd writer"
            )
        ...
        self.low_state = None
        self.crc = CRC()
        self._writes_armed = self.config.get("ENV_TYPE") == "sim"
        self._command_prepared = False
        self.write_count = 0

    @property
    def writes_armed(self) -> bool:
        return self._writes_armed

    def arm_writes(self, *, takeover_confirmed: bool = False) -> None:
        if self.config.get("ENV_TYPE") == "real" and not takeover_confirmed:
            raise PermissionError("real lowcmd writes require explicit whole-body takeover confirmation")
    ...
    def prepare_command(self, cmd_q: np.ndarray, cmd_dq: np.ndarray, cmd_tau: np.ndarray) -> None:
        expected = int(self.config["NUM_JOINTS"])
        values = {
            "q": np.asarray(cmd_q, dtype=np.float64),
            "dq": np.asarray(cmd_dq, dtype=np.float64),
            "tau": np.asarray(cmd_tau, dtype=np.float64),
        }
        for name, value in values.items():
            if value.shape != (expected,) or not np.isfinite(value).all():
                raise ValueError(f"cmd_{name} must be finite shape ({expected},)")
        for i in range(self.config["NUM_MOTORS"]):
            ...
            if joint_index == -1:
                ...
                self.low_cmd.motor_cmd[motor_index].q = values["q"][joint_index]
                self.low_cmd.motor_cmd[motor_index].dq = values["dq"][joint_index]
                self.low_cmd.motor_cmd[motor_index].tau = values["tau"][joint_index]
            # kp kd
    ...
    def write_prepared(self) -> None:
        if not self._writes_armed:
            raise PermissionError("lowcmd Write blocked while command sender is disarmed")
        if not self._command_prepared:
            raise RuntimeError("no validated lowcmd has been prepared")
        self.lowcmd_publisher_.Write(self.low_cmd)
        self.write_count += 1

    def send_command(self, cmd_q: np.ndarray, cmd_dq: np.ndarray, cmd_tau: np.ndarray):
        self.prepare_command(cmd_q, cmd_dq, cmd_tau)
        self.write_prepared()
...
class HandCommandSender:
    def __init__(self, is_left: bool = True, *, env_type: str):
        if env_type != "sim":
            raise RuntimeError(
                "Dex3 command sender is simulation-only in the Decoupled WBC path"
            )
